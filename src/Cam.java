...
public class Cam {
    ...
    private double[] Accerleration = new double[2]; // Acceleration of the Cam Index 0 = x Index 1 = y
    ...
    public void update(int[] PlayerPos){ // Update Physics
        ...
        Accerleration[0] = Math.pow(Force[0],2) * mass; // Update Acceleration
        Accerleration[1] = Math.pow(Force[1],2) * mass;
        Speed[0] = Speed[0] + (int)Accerleration[0] * 0.01; // Update Speed this is dependent on the delta Time 0,01 s == 10 ms
        Speed[1] = Speed[1] + (int)Accerleration[1] * 0.01;
